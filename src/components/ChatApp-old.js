...
export default function ChatApp() {
  ...
  //   socketRef.current = new WebSocket(`ws://localhost:8000/ws/chat/${uid}`);
  ...
  //     setChat(prev => [...prev, { from: 'them', text: data.text }]);
  //   };
  ...
  useEffect(() => {
    ...
    const fetchInitialContacts = async () => {
      try {
        ...
        // Save all contact messages to IndexedDB
        for (const contact of res.data.contacts) {
          const msgs = (contact.recent_messages || []).map(msg => ({
            id: msg.id,
  ...
  useEffect(() => {
    ...
    socketRef.current.onmessage = async (event) => {
      ...
      // 🧠 Receiver-side message transformation based on settings
      if (!isMe) {
        const receiveFormat = settings.chatFormat;
        const receiveLang = settings.receivingLang;
        console.log("receiveFormat",receiveFormat, "and receiveLang",receiveLang ,"and newMsg.type",newMsg.type )
        // Convert Audio → Text if receiver wants text
        if (receiveFormat === 'Text' && newMsg.type === 'Audio') {
          try {
            const formData = new FormData();
            formData.append('audio_url', data.audio_url); // Ensure backend supports this
            const res = await fetch(`http://${process.env.REACT_APP_API_BASE_URL}/speech_to_text`, {
              method: 'POST',
              body: formData
            });
            const result = await res.json();
            newMsg.text = result.transcribed_text || '[Audio transcription failed]';
            newMsg.type = 'text';
          } catch (err) {
            console.error("❌ Audio-to-text failed", err);
          }
        }

        // Convert Text → Audio if receiver wants audio
        if (receiveFormat === 'Audio' && newMsg.type === 'text') {
          try {
            const formData = new FormData();
            formData.append('text', newMsg.text);
            formData.append('language', receiveLang);
            const res = await fetch(`http://${process.env.REACT_APP_API_BASE_URL}/text-to-audio`, {
              method: 'POST',
              body: formData
            });
            const result = await res.json();
            newMsg.audioUrl = result.audio_url;
            newMsg.type = 'audio';
            newMsg.text = '';
          } catch (err) {
            console.error("❌ Text-to-audio failed", err);
          }
        }
      }
  ...
  try {
    // 1. Upload blob to backend
    const formData = new FormData();
    formData.append('file', blob, 'voice_message.webm');

    const res = await fetch(`http://${process.env.REACT_APP_API_BASE_URL}/upload-audio`, {
      method: 'POST',
      body: formData,
    });
    const result = await res.json();
                  ...
                  {selectedContact && messages[selectedContact?.uid]?.map((msg, idx) => {
                    ...
                    return (
                      ...
                      <div key={idx} className={`d-flex mb-2 ${isCurrentUser ? 'justify-content-end' : 'justify-content-start'}`}>
                        <div className={`message-bubble ${isCurrentUser ? 'bg-success text-white' : 'bg-white border'}`} style={{ maxWidth: '70%' }}>
