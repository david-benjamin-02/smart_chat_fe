...
export default function ChatApp() {
  ...
  const handleCallSignal = async (data) => {
    const { type, from, data: signalData } = data;
  ...
  //   const socket = new WebSocket(`ws://${process.env.REACT_APP_API_BASE_URL}/ws/call/${uid}`);
  ...
  //   socket.onclose = () => {
  //     console.warn("❌ Call signaling socket closed");
  //   };
  ...
  const ensureCallSocketConnected = () => {
    const socket = callSocketRef.current;

    if (!socket || socket.readyState === WebSocket.CLOSED || socket.readyState === WebSocket.CLOSING) {
      console.log("🔁 Reconnecting call WebSocket...");

      const newSocket = new WebSocket(`ws://${process.env.REACT_APP_API_BASE_URL}/ws/call/${uid}`);
      callSocketRef.current = newSocket;

      newSocket.onopen = () => console.log("📞 Call WebSocket reconnected");
      newSocket.onmessage = (e) => handleCallSignal(JSON.parse(e.data));
      newSocket.onclose = () => console.warn("⚠️ Call WebSocket closed again");
  ...
  useEffect(() => {
    ...
    socket.onopen = () => console.log("📞 Call signaling socket connected");
    socket.onmessage = (event) => {
      const data = JSON.parse(event.data);
      handleCallSignal(data);
    };
    socket.onclose = () => console.log("📞 Call signaling socket disconnected");
  ...
  const startCall = async () => {
    console.log("📞 Starting call with:", selectedContact?.uid);

    const isReady = ensureCallSocketConnected();
    if (!isReady) {
      alert("Call socket not ready. Please try again in a second.");
      return;
    }
    ...
    peerConnection.onicecandidate = (event) => {
      if (event.candidate && callSocketRef.current.readyState === WebSocket.OPEN) {
        callSocketRef.current.send(JSON.stringify({
    ...
    console.log("📤 Sent offer to:", selectedContact.uid);
          ...
          {selectedContact ? (
            <>
              ...
              <div className="p-3 border-bottom bg-white d-flex align-items-center justify-content-between">
                {/* Left side: Avatar + name + typing + add/cancel */}
                ...
                {!selectedContact?.is_unknown && (
                  <button onClick={startCall} className="btn btn-sm btn-outline-primary">
